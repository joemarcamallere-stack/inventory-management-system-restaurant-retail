...
import { createUser, deleteUser, updateUser } from '../../app/api/client';
import type {
  InventoryItem,
  PurchaseOrder,
  ProductReceived,
                  ...
                  <div key={po.id} className="border border-[rgba(0,0,0,0.1)] rounded-[12px] p-4 hover:bg-[#F8FAFB] transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-[13px] text-[#323B42]">Items: {po.items.length} | Total: â‚±{po.totalAmount.toLocaleString()}</p>
                      </div>
                      <button
          ...
          <div className="bg-white rounded-[14px] p-6 max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6 bg-[#E0F5F1] border border-[#007A5E] rounded-[12px] p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[14px] font-semibold text-[#007A5E] mb-1">ðŸ¤– Auto-Sorting Enabled</p>
                  ...
                  <div key={idx} className="bg-[#F8FAFB] border border-[rgba(0,0,0,0.1)] rounded-[12px] p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-[13px] text-[#323B42]">Ordered: {item.quantity} units @ â‚±{item.price} each</p>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full px-3 py-2 border border-[rgba(0,0,0,0.1)] rounded-[8px] text-[14px] focus:outline-none focus:border-[#007A5E]"
                        >
                          <option value="Excellent">Excellent âœ¨</option>
                          <option value="Good">Good ðŸ‘</option>
                          <option value="Fair">Fair âš ï¸</option>
                          <option value="Damaged">Damaged âŒ</option>
                        </select>
                      </div>
                    </div>
                    ...
                    {inspection.rejectedQty > 0 && (
                      <div className="mt-3 p-3 bg-[#fff4e6] border border-[#FFA500] rounded-[8px]">
                        <p className="text-[13px] text-[#d08700] font-medium">
                          âš ï¸ {inspection.rejectedQty} unit(s) will be rejected
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="text-right">
                          <p className="text-[13px] text-[#323B42]">
                            ...
                            {item.rejectedQty > 0 && (
                              <> â€¢ <span className="font-semibold text-[#E7000B]">{item.rejectedQty}</span> rejected</>
                            )}
                          </p>
    ...
    <div>
                  ...
                  {bundleForm.items.map((bundleItem) => {
                    ...
                    return item ? (
                      <div key={bundleItem.itemId} className="flex items-center justify-between bg-[#F8FAFB] rounded-[8px] px-4 py-3">
                        <div className="flex-1">
                          <p className="text-[14px] font-medium text-[#323B42]">{item.name}</p>
                          <p className="text-[12px] text-[#6b7280]">{item.category} â€¢ â‚±{item.price}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                            <button
                          ...
                          <span className="text-[14px] font-semibold text-[#323B42] w-20 text-right">
                            â‚±{(item.price * bundleItem.quantity).toLocaleString()}
                          </span>
                          <button
              ...
              <div className="bg-[#F3F4F6] rounded-[12px] p-4 mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[16px] font-medium text-[#323B42] line-through">
                    â‚±{calculateBundlePrice(bundleForm.items, 0).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[14px] text-[#323B42]">Discount ({bundleForm.discount}%):</span>
                  <span className="text-[16px] font-medium text-[#E7000B]">
                    -â‚±{(calculateBundlePrice(bundleForm.items, 0) - calculateBundlePrice(bundleForm.items, bundleForm.discount)).toLocaleString()}
                  </span>
                </div>
                <div className="border-t border-[rgba(0,0,0,0.2)] pt-2 flex justify-between items-center">
                  ...
                  <span className="text-[24px] font-bold text-[#007A5E]">
                    â‚±{calculateBundlePrice(bundleForm.items, bundleForm.discount).toLocaleString()}
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-[14px] p-6 max-w-lg w-full">
            ...
            <div className="mb-4 p-4 bg-[#F8FAFB] rounded-[8px]">
              ...
              <p className="text-[16px] font-bold text-[#007A5E] mt-2">Price: â‚±{selectedBundle.price.toLocaleString()}</p>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-[12px] text-[#6b7280] mt-1">
                          {item.subcategory} â€¢ {item.quantity} available â€¢ â‚±{item.price}
                        </p>
                      </div>
                      <button
                        onClick={() => {
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-[rgba(0,0,0,0.1)] rounded-[14px] p-4">
          ...
          <p className="text-[#007A5E] text-[24px] font-bold">â‚±{stats.totalValue.toLocaleString()}</p>
        </div>
      </div>
              ...
              <div key={bundle.id} className="bg-white border border-[rgba(0,0,0,0.1)] rounded-[14px] p-5 hover:shadow-md transition-shadow flex flex-col">
                    ...
                    {bundle.items.map((bundleItem) => {
                      ...
                      return item ? (
                        <div key={bundleItem.itemId} className="flex justify-between text-[12px] gap-2">
                          <span className="text-[#323B42] line-clamp-1 flex-1">{item.name} Ã— {bundleItem.quantity}</span>
                          <span className="text-[#6b7280] shrink-0">â‚±{(item.price * bundleItem.quantity).toLocaleString()}</span>
                        </div>
                      ) : null;
                    })}
                  </div>
                ...
                <div className="border-t border-[rgba(0,0,0,0.1)] pt-3 mb-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[13px] text-[#6b7280] line-through">â‚±{originalPrice.toLocaleString()}</span>
                  </div>
                  <div className="flex items-baseline justify-between">
                    <span className="text-[13px] font-semibold text-[#323B42]">Bundle Price:</span>
                    <span className="text-[22px] font-bold text-[#007A5E]">â‚±{bundle.price.toLocaleString()}</span>
                  </div>
                  <p className="text-[11px] text-[#00a63e] font-medium text-right">You save â‚±{savings.toLocaleString()}</p>
                </div>
