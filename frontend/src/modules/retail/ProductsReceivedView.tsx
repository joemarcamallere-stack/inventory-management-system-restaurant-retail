  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-[30px] font-bold text-foreground">Products Received</h2>
          <p className="text-foreground text-[14px] mt-1">Inspect and log received inventory shipments</p>
        </div>
        <button
      ...
      {showReceiveModal && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-[14px] p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[24px] font-bold text-foreground">Select Purchase Order to Receive</h3>
              <button onClick={() => setShowReceiveModal(false)} className="p-2 hover:bg-muted rounded">
                <X className="size-5 text-foreground" />
              </button>
            </div>

            {approvedPOs.length === 0 ? (
              <p className="text-center py-8 text-foreground">No approved purchase orders available</p>
            ) : (
              <div className="space-y-3">
                {approvedPOs.map(po => (
                  <div key={po.id} className="border border-border rounded-[12px] p-4 hover:bg-muted transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="text-[16px] font-semibold text-foreground">{po.orderNumber}</h4>
                          <span className="px-2 py-1 rounded text-[11px] font-semibold bg-secondary/10 text-secondary">
                            APPROVED
                          </span>
                        </div>
                        <p className="text-[13px] text-foreground">Supplier: {po.supplier?.name ?? 'N/A'}</p>
                        <p className="text-[13px] text-foreground">Date: {new Date(po.createdAt).toLocaleDateString()}</p>
                        <p className="text-[13px] text-foreground">Items: {(po.items ?? []).length} | Total: ₱{(po.totalAmount ?? 0).toLocaleString()}</p>
                      </div>
                      <button
                        onClick={() => handleStartReceiving(po)}
                        className="px-4 py-2 bg-secondary text-white rounded-[8px] text-[13px] font-medium hover:bg-secondary transition-colors"
                      >
                        Start Receiving
                      </button>
                    </div>
      ...
      {showInspectionModal && selectedPO && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-[14px] p-6 max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-[24px] font-bold text-foreground">Quality Inspection - {selectedPO.orderNumber}</h3>
                <p className="text-[14px] text-foreground mt-1">Supplier: {selectedPO.supplier?.name ?? 'N/A'}</p>
              </div>
              <button onClick={() => setShowInspectionModal(false)} className="p-2 hover:bg-muted rounded">
                <X className="size-5 text-foreground" />
              </button>
            </div>

            <div className="mb-6 bg-secondary/10 border border-secondary rounded-[12px] p-4">
              <div className="flex items-start gap-3">
                <CheckCircle className="size-5 text-secondary mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-[14px] font-semibold text-secondary mb-1">Auto-Sorting Enabled</p>
            ...
            <div className="space-y-4">
              {(selectedPO.items ?? []).map((item: any) => {
                ...
                return (
                  <div key={item.id} className="bg-muted border border-border rounded-[12px] p-5">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <h4 className="text-[16px] font-semibold text-foreground">{item.name}</h4>
                        <p className="text-[13px] text-foreground">Ordered: {item.quantity} units @ ₱{item.unitPrice ?? 0} each</p>

                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <span className="text-[11px] text-muted-foreground">Auto-Sort:</span>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                            autoSort.confidence === 'high' ? 'bg-secondary/10 text-secondary' :
                            autoSort.confidence === 'medium' ? 'bg-warning/10 text-warning' :
                          ...
                          {autoSort.confidence === 'low' && (
                            <span className="text-[10px] text-destructive">(Low Confidence - May need review)</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-4">
                      <div>
                        <label className="block text-[12px] font-medium text-foreground mb-2">Received Qty *</label>
                        <input
                          type="number"
                          min="0"
                          max={item.quantity}
                          ...
                          className="w-full px-3 py-2 border border-border rounded-[8px] text-[14px] focus:outline-none focus:border-secondary"
                        />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled
                          className="w-full px-3 py-2 border border-border rounded-[8px] text-[14px] bg-muted cursor-not-allowed"
                        />
                      </div>
                      <div>
                        <label className="block text-[12px] font-medium text-foreground mb-2">
                          ...
                          {autoSort.suggestedCondition && autoSort.suggestedCondition !== inspection.condition && (
                            <span className="ml-2 text-[10px] text-secondary font-normal">
                    ...
                    <div>
                      <label className="block text-[12px] font-medium text-foreground mb-2">Inspection Notes</label>
                      <textarea
                        value={inspection.inspectionNotes}
                        onChange={(e) => handleInspectionChange(item.id, 'inspectionNotes', e.target.value)}
                        className="w-full px-3 py-2 border border-border rounded-[8px] text-[14px] focus:outline-none focus:border-secondary resize-none"
                        rows={2}
                    ...
                    {inspection.rejectedQty > 0 && (
                      <div className="mt-3 p-3 bg-warning/10 border border-warning rounded-[8px]">
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                disabled={saving}
                className="flex-1 px-4 py-2 border border-border rounded-[8px] text-[14px] font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-border rounded-[14px] p-4">
          <p className="text-foreground text-[12px] mb-1">Ready to Receive</p>
          <p className="text-warning text-[24px] font-bold">{loading ? '—' : stats.readyToReceive}</p>
        </div>
        <div className="bg-white border border-border rounded-[14px] p-4">
          <p className="text-foreground text-[12px] mb-1">Fully Accepted</p>
          <p className="text-secondary text-[24px] font-bold">{loading ? '—' : stats.fullyAccepted}</p>
        </div>
        <div className="bg-white border border-border rounded-[14px] p-4">
          <p className="text-foreground text-[12px] mb-1">With Rejections</p>
          <p className="text-secondary text-[24px] font-bold">{loading ? '—' : stats.withRejections}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white border border-border rounded-[14px] p-12 text-center">
            <PackageCheck className="size-16 text-muted mx-auto mb-4" />
            <p className="text-[16px] text-foreground font-medium">No receipts found</p>
            <p className="text-[14px] text-muted-foreground mt-1">Start receiving purchase orders to see them here</p>
          </div>
        ) : (
          filteredReceipts.map(po => {
            ...
            return (
              <div key={po.id} className="bg-white border border-border rounded-[14px] p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-[18px] font-semibold text-foreground">{po.orderNumber}</h3>
                      <span className={`px-3 py-1 rounded text-[12px] font-semibold ${
                        status === 'Fully Accepted' ? 'bg-secondary/10 text-secondary' :
                        'bg-secondary/10 text-secondary'
                      }`}>
                        {status}
                      </span>
                    </div>
                    <p className="text-[14px] text-foreground">Supplier: <span className="font-medium">{po.supplier?.name ?? 'N/A'}</span></p>
                    <p className="text-[14px] text-foreground">Date Received: {po.receivedAt ? new Date(po.receivedAt).toLocaleDateString() : 'N/A'}</p>
                    <p className="text-[14px] text-foreground">Received By: {po.receivedBy?.name ?? 'N/A'}</p>
                  </div>
                  <div className="text-right">
                    <div className="bg-secondary/10 rounded-[8px] px-4 py-2 mb-2">
                      <p className="text-[11px] text-foreground">Accepted</p>
                      <p className="text-[20px] font-bold text-secondary">{totalAccepted}</p>
                    </div>
                    {totalRejected > 0 && (
                      <div className="bg-destructive/10 rounded-[8px] px-4 py-2">
                        <p className="text-[11px] text-foreground">Rejected</p>
                        <p className="text-[20px] font-bold text-destructive">{totalRejected}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-4">
                  ...
                  <div className="space-y-2">
                    {(po.items ?? []).map((item: any) => {
                      ...
                      return (
                        <div key={item.id} className="bg-muted rounded-[8px] p-4">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <div className="text-right">
                              <p className="text-[13px] text-foreground">
                                <span className="font-semibold text-secondary">{accepted}</span> accepted
                                {item.rejectedQty > 0 && (
                                  <> • <span className="font-semibold text-destructive">{item.rejectedQty}</span> rejected</>
                                )}
                              </p>
                              <p className="text-[12px] text-muted-foreground">Ordered: {item.quantity} | Received: {item.receivedQty}</p>
                            </div>
                          </div>
                        </div>
                      );
